...
export function bindNodeEvents() {
   ...
   nodes.forEach((node) => {
      const schema = ERDState.erdData.find(
         (s) => s.table === node.dataset.table,
      );
      if (!schema) return;
      ...
      header.addEventListener('mousedown', (e) => {
         ...
         if (!ERDState.selectedTables.has(schema.table)) {
            ...
            document.querySelectorAll('.erd-node').forEach((n) => {
               if (ERDState.selectedTables.has(n.dataset.table))
      ...
      if (schema.isDraft) {
         ...
         header.addEventListener('dblclick', (e) => {
            ...
            const sidebarBtn = document.querySelector(
               `.table-btn[data-table="${node.dataset.table}"]`,
            );
            if (sidebarBtn) {
                  ...
                  action: () => {
                     ...
                     ERDState.erdData.forEach((s) => {
                        s.columns.forEach((c) => {
                           if (
                              c.fkTarget &&
                              c.fkTarget.table === schema.table &&
                              c.fkTarget.column === colData.name
                           ) {
                              delete c.fkTarget;
                           }
                        });
         ...
         editables.forEach((el) => {
            el.addEventListener('dblclick', (e) => {
               ...
               if (field === 'type') {
                  openTypePicker({
                     anchorEl: el,
                     initialValue: colData.type,
                     dialect: window.AppState?.dbType,
                     onSelect: (newType) => {
                        if (newType && newType !== colData.type) {
                           ERDState.pushUndoState();
                           colData.type = newType;
                           renderNodes();
                        }
                     },
                  });
               ...
               const commit = () => {
                  ...
                  let newValue =
                     field === 'type' ? input.value.toUpperCase() : input.value;
         ...
         if (dragHandle) {
            dragHandle.addEventListener('mousedown', (e) => {
               ...
               ERDState.drawingFkStartCoords = {
                  ...
                  y:
                     (rect.top + rect.height / 2 - workspaceRect.top) /
                     ERDState.zoom,
               };
            });
         }
      ...
      cols.forEach((colEl) => {
         colEl.addEventListener('dragover', (e) => {
            if (
               !ERDState.draggingColInfo ||
               ERDState.draggingColInfo.table !== schema.table
            )
               return;
            e.preventDefault();
...
export function bindNodeGlobalEvents() {
   ...
   document.addEventListener('mousemove', (e) => {
      if (ERDState.drawingFkStartCol) {
         ...
         if (!tempPath) {
            tempPath = document.createElementNS(
         ...
         if (tempPath)
            tempPath.setAttribute(
               'd',
               `M ${ERDState.drawingFkStartCoords.x} ${ERDState.drawingFkStartCoords.y} L ${curX} ${curY}`,
            );
         return;
      }
      ...
      if (ERDState.drawingFkStartCol) {
         ...
         if (targetNode) {
            ...
            if (targetTable !== ERDState.drawingFkStartCol.table) {
               const targetSchema = ERDState.erdData.find(
                  (s) => s.table === targetTable,
               );
               const pkCol = targetSchema.columns.find((c) => c.isPk);
               if (pkCol) {
                  const isNullable = confirm(
                     `Create Foreign Key to ${targetTable}?\n\nClick OK for Nullable (0..N)\nClick Cancel for Not Nullable (1..N)`,
                  );

                  ERDState.pushUndoState();
                  const sourceSchema = ERDState.erdData.find(
                     (s) => s.table === ERDState.drawingFkStartCol.table,
                  );
                  const sourceCol =
                     sourceSchema.columns[ERDState.drawingFkStartCol.colIndex];
                  sourceCol.fkTarget = {
                     table: targetTable,
                     column: pkCol.name,
                  };
                  ...
                  sourceSchema.columns.splice(
                     ERDState.drawingFkStartCol.colIndex,
                     1,
                  );
